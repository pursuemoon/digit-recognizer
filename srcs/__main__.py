...
if __name__ == "__main__":
    ...
    # network.add_layer(LinearLayer(input_dim=1*28*28, output_dim=512, act_func=ActFunc.Relu))
    # network.add_layer(LinearLayer(input_dim=512, output_dim=256, act_func=ActFunc.Relu))
    # network.add_layer(LinearLayer(input_dim=256, output_dim=10, act_func=ActFunc.Softmax))
    ...
    # network.add_layer(Conv2dLayer(input_shape=(1,28,28), kernel_size=5, filter_num=4, stride=3, padding=1, act_func=ActFunc.Relu))
    # network.add_layer(LinearLayer(input_dim=4*9*9, output_dim=10, act_func=ActFunc.Softmax))

    network.add_layer(Conv2dLayer(input_shape=(1,28,28), kernel_size=5, filter_num=12, stride=2, padding=1, act_func=ActFunc.Relu))
    network.add_layer(Conv2dLayer(input_shape=(12,13,13), kernel_size=5, filter_num=20, stride=2, padding=0, act_func=ActFunc.Relu))
    network.add_layer(LinearLayer(input_dim=20*5*5, output_dim=128, act_func=ActFunc.Relu))
    ...
    network.train(max_epoch=40, learning_rate=0.001, regular_coef=0.001,
